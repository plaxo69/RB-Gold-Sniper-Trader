...
async function yahoo(timeframe) {
  ...
  // Yahoo currently exposes the gold futures contract reliably; it is used only as the data feed.
  // The TradingView chart remains OANDA:XAUUSD for the user's visual spot chart.
  const symbol="GC=F";
  const url=`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}`;
  const response=await axios.get(url,{params:{interval:cfg.interval,range:cfg.range,includePrePost:true,events:"div,splits"},timeout:10000,headers:{"User-Agent":"Mozilla/5.0 RB-Gold-Sniper/2.1"}});
...
app.get("/api/health",async(_req,res)=>{try{const m=await market("1m");res.json({status:"OK",marketConnected:true,source:m.source,symbol:m.symbol,aiEnabled:false,mode:"ALERTAS",timestamp:new Date().toISOString()})}catch(e){res.status(503).json({status:"DEGRADED",marketConnected:false,aiEnabled:false,mode:"ALERTAS",error:e.message})}});
...
app.get("/api/settings",(_req,res)=>res.json({success:true,settings:{mode:"ALERTAS",aiEnabled:false,source:"Yahoo Finance",symbol:"GC=F",chart:"OANDA:XAUUSD"}}));
