(()=>{
  ...
  function ensure(){
    ...
    el.innerHTML='<strong id="ai-confidence-value">🧠 Confiança IA: —</strong><span id="ai-confidence-note" style="font-size:.68rem;color:#9aa5bd">parecer independente</span>';
    target.parentElement.insertBefore(el,target);
    return el;
  }
  function set(text,good=false,note=""){
    const el=ensure();if(!el)return;
    const value=el.querySelector("#ai-confidence-value")||el.querySelector("strong");
    const noteEl=el.querySelector("#ai-confidence-note");
    if(value){value.textContent=text;value.style.color=good?"#35d07f":"#ffc107"}
  ...
  async function run(){
    ...
    try{
      ...
      if(!health.aiEnabled){set("🧠 Confiança IA: indisponível",false,"IA indisponível — OPENAI_API_KEY não está disponível no ambiente de produção.");return}
      ...
      if(d.available&&Number.isFinite(Number(d.confidence))){set(`🧠 Confiança IA: ${Math.round(Number(d.confidence))}%${d.opinion?` • ${d.opinion}`:""}`,true,d.note||"parecer independente")}
