...
function analyze(main, m5, m15, h1) {
  if(main.length<60 || m5.length<30 || m15.length<30 || h1.length<30) return null;
  const closes=main.map(x=>x.close), last=closes.at(-1), prev=closes.at(-2);
  const r=rsi(closes), e20=ema(closes,20), e50=ema(closes,50), e200=ema(closes,200), a=atr(main), s=structure(main);
  const t5=trend(m5), t15=trend(m15), t1=trend(h1);
  let buy=0,sell=0; const buyReasons=[],sellReasons=[];
  ...
  if(type!=="WAIT"&&a>0){
    if(type==="BUY"){sl=Math.min(s.support-a*.25,last-a*1.25);const risk=last-sl;tp=last+Math.max(risk*2,a*2);rr=(tp-last)/risk;}
    else{sl=Math.max(s.resistance+a*.25,last+a*1.25);const risk=sl-last;tp=last-Math.max(risk*2,a*2);rr=(last-tp)/risk;}
  }
  return {type,score,confidence,price:last,rsi:r,ema20:e20,ema50:e50,ema200:e200,atr:a,support:s.support,resistance:s.resistance,trend5:t5,trend15:t15,trend1:t1,sl,tp,rr,reasons,source:"candles reais"};
}
...
function saveSignal(a) {
  ...
  state.history.unshift(item); state.history=state.history.slice(0,100);
...
function renderAnalysis(a) {
  ...
  if(a.type==="WAIT"){box.innerHTML=`<div class="no-trades">⏳ WAIT — sem convergência Sniper suficiente. Última análise: ${a.price.toFixed(2)}</div>`;return;}
...
function renderHistory() {
  const total=state.history.length,wins=state.history.filter(x=>x.result==="WIN").length;
  $("total-trades").textContent=total; $("total-profit").textContent="—"; $("win-rate").textContent=total?`${((wins/total)*100).toFixed(1)}%`:"—";
}
...
function clearTrades() {
  state.history=[]; localStorage.removeItem("rb_sniper_history"); renderHistory();
