...
function initTradingView(){const host=$("tradingview-chart");if(!host||!window.TradingView)return;state.tvWidget=new TradingView.widget({autosize:true,symbol:"OANDA:XAUUSD",interval:tvInterval(state.timeframe),timezone:"Europe/Lisbon",theme:"dark",style:"1",locale:"pt",enable_publishing:false,hide_top_toolbar:false,hide_legend:false,allow_symbol_change:false,save_image:false,container_id:"tradingview-chart"})}
function changeTradingViewInterval(tf){if(state.tvWidget?.chart)state.tvWidget.chart().setResolution(tvInterval(tf))}
function updateUI(d){const last=d.candles.at(-1);$("current-price").textContent=`$${last.close.toFixed(2)}`;$("current-bid").textContent=`$${last.close.toFixed(2)}`;$("current-ask").textContent=`$${last.close.toFixed(2)}`;$("current-spread").textContent=(last.high-last.low).toFixed(2);$("server-info").textContent=`XAUUSD • ${d.source} • ${d.timeframe} • ${new Date(d.timestamp).toLocaleTimeString("pt-PT")}`}
function logAudit(a){if(!a)return;const today=new Date().toISOString().slice(0,10);if(!a.signalTime?.startsWith(today))return;const key=`${a.signalTime}-${a.type}-${a.score}-${a.structure5}-${a.structure15}-${a.structure1}`;if(state.audit.some(x=>x.key===key))return;state.audit.unshift({key,time:a.signalTime,price:a.price,type:a.type,score:a.score,confidence:a.confidence,reason:a.reasons?.join(" • ")||"",rsi:a.rsi,structure5:a.structure5,structure15:a.structure15,structure1:a.structure1,buyScore:a.buyScore??null,sellScore:a.sellScore??null});state.audit=state.audit.slice(0,2000);localStorage.setItem("rb_sniper_audit",JSON.stringify(state.audit))}
async function saveSignal(a){if(!a||a.type==="WAIT")return;const key=`${a.type}-${a.signalTime}`;if(key===state.lastSignalKey)return;state.lastSignalKey=key;const item={...a,id:Date.now(),time:new Date().toISOString(),status:"ALERTA",result:null};state.history.unshift(item);state.history=state.history.slice(0,100);localStorage.setItem("rb_sniper_history",JSON.stringify(state.history));renderHistory();notify(`🎯 ${a.type} ${a.confidence}% — XAUUSD ${a.price.toFixed(2)}`,"success");try{const r=await fetch(`${API_BASE}/trades`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(item)});const d=await r.json();if(d?.trades){state.history=d.trades.slice(-100).reverse();localStorage.setItem("rb_sniper_history",JSON.stringify(state.history));renderHistory()}}catch(_){}}
...
async function refresh(){if(state.busy)return;state.busy=true;try{const [m1,m5,m15,h1]=await Promise.all(["1m","5m","15m","1h"].map(fetchTF));updateUI(state.candles[state.timeframe]||m1);const a=evaluate(m1.candles,m5.candles,m15.candles,h1.candles);renderAnalysis(a);logAudit(a);if(state.running)saveSignal(a);state.lastRefresh=new Date();$("status").textContent=`🟢 XAUUSD • mercado aberto • ${state.lastRefresh.toLocaleTimeString("pt-PT")}`;$("status").className="status-indicator online";renderHistory()}catch(e){$("status").textContent="🔴 XAUUSD indisponível";$("status").className="status-indicator offline";$("server-info").textContent=e.message}finally{state.busy=false}}
function startSniper(){state.running=true;clearInterval(state.refreshTimer);refresh();state.refreshTimer=setInterval(refresh,15000)}
...
async function clearTrades(){state.history=[];localStorage.removeItem("rb_sniper_history");renderHistory();$("trades-container").innerHTML='<div class="no-trades">A limpar histórico…</div>';try{const r=await fetch(`${API_BASE}/trades`,{method:"DELETE"});if(!r.ok)throw new Error("Falha no histórico persistente");$("trades-container").innerHTML='<div class="no-trades">Histórico limpo.</div>'}catch(_){$("trades-container").innerHTML='<div class="no-trades">Histórico local limpo; persistente não disponível.</div>'}}
...
window.addEventListener("load",()=>{renderHistory();document.querySelectorAll(".timeframe-btn").forEach(b=>b.addEventListener("click",()=>changeTimeframe(b.dataset.tf)));$("status").textContent="🟡 A obter XAUUSD real…";initTradingView();startSniper()});
