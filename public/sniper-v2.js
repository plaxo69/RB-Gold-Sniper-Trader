...
function structureSignal(c,p=C.structureLookback){const x=c.slice(-(p+1),-1);if(x.length<p+1)return{bosBuy:false,bosSell:false,chochBuy:false,chochSell:false};const last=x.at(-1),prior=x.slice(0,-1),hi=Math.max(...prior.map(v=>v.high)),lo=Math.min(...prior.map(v=>v.low));const mid=Math.floor(prior.length/2),left=prior.slice(0,mid),right=prior.slice(mid);const leftHi=Math.max(...left.map(v=>v.high)),rightHi=Math.max(...right.map(v=>v.high)),leftLo=Math.min(...left.map(v=>v.low)),rightLo=Math.min(...right.map(v=>v.low));const bosBuy=last.close>hi,bosSell=last.close<lo;const chochBuy=last.close>rightHi&&rightLo<leftLo;const chochSell=last.close<rightLo&&rightHi>leftHi;return{bosBuy,bosSell,chochBuy,chochSell}}
function analyze(main,m3,m5,m15,h1){
 ...
 const ms=structureSignal(closed),buyBreak=sig.close>S.resistance&&prev.close<=S.resistance,buyNear=sig.low<=e20+A*.45&&sig.close>e20&&sig.close>sig.open;
