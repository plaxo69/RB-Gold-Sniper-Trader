(function(root){
const C={period:20,minHistory:205,minRR:2,minBodyAtr:.10,maxExtAtr:2.6,reversalSupportAtr:1.8,reversalWickBody:1.05};
...
function analyze(main,m3,m5,m15,h1){
 ...
 const rej=rejection(sig),supportBounce=sig.low<=S.support+A*C.reversalSupportAtr&&sig.close>sig.open&&rej.bull&&R>=32&&R<=66&&Number.isFinite(prevR)&&R>prevR;
 ...
 const buySetup=(buyTrend&&buyMom&&extBuy<=C.maxExtAtr&&(buyBreak||buyNear))||momentumBuy||reversalBuy;
 const sellSetup=(sellTrend&&sellMom&&extSell<=C.maxExtAtr&&(sellBreak||sellNear))||momentumSell||reversalSell;
 ...
 const setupPass=buyBreak||buyNear||sellBreak||sellNear||supportBounce||resistanceReject||momentumBuy||momentumSell;
 ...
 const rej=[];if(!atrOK)rej.push('volatilidade fora da faixa');if(!bodyOK)rej.push('candle fraco');if(!buyTrend&&!sellTrend&&!momentumBuy&&!momentumSell&&!reversalBuy&&!reversalSell)rej.push('sem direção confirmada');if(!setupPass)rej.push('sem breakout/pullback/rejeição confirmado');if(!buyMom&&!sellMom)rej.push('RSI sem momentum');
 return{type:finalType,score:passed,confidence:finalType==='WAIT'?0:Math.min(95,72+passed*4),price,signalTimestamp:sig.timestamp||null,rsi:R,ema20:e20,ema50:e50,ema200:e200,atr:A,support:S.support,resistance:S.resistance,trend3:t3,trend5:t5,trend15:t15,trend1:t1,sl:sl==null?null:round(sl),tp:tp==null?null:round(tp),rr:rr==null?null:rr,reasons:finalType==='BUY'?["M1 confirmou movimento comprador",buyBreak?'breakout fechado':buyNear?'pullback EMA20':reversalBuy?'rejeição de suporte + RSI a recuperar':'momentum comprador',"H1 não contrário","gestão RR 1:2+"]:finalType==='SELL'?["M1 confirmou movimento vendedor",sellBreak?'breakdown fechado':sellNear?'pullback EMA20':reversalSell?'rejeição de resistência + RSI a perder força':'momentum vendedor',"H1 não contrário","gestão RR 1:2+"]:[],rejectionReasons:rej,source:'candles reais fechados',filters:{marketIsTradable:atrOK,bodyIsDecisive:bodyOK,buyTrend,sellTrend,buyBreakout:buyBreak,sellBreakdown:sellBreak,buyMomentum:buyMom,sellMomentum:sellMom,buyEntryIsControlled:extBuy<=C.maxExtAtr,sellEntryIsControlled:extSell<=C.maxExtAtr,reversalBuy,reversalSell,supportBounce,resistanceReject,momentumBuy,momentumSell,technicalPass:atrOK&&bodyOK,m3Buy:t3==='ALTA',m3Sell:t3==='BAIXA'}};
}
